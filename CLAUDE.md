...
**Reference manifest** (`docs/references/manifest.json`):
- Generated by the three reference collector skills during Phase 0
- Three optional top-level sections: `art`, `audio`, `narrative`
- Each section populated when its corresponding collector skill runs
- The manifest is the GameForge-readable parallel to the agent-readable direction docs
- Style anchor paths reference files in `assets/references/`, NOT `docs/references/`
- Audio references use Epidemic Sound track UUIDs, not file paths
- Do NOT manually edit — regenerate via the collector skills

**Playtest guides** (`docs/sprint-logs/sprint-{N}-playtest-guide.md`):
- Generated at Phase D of every sprint by the orchestrator
- YAML frontmatter with `type: playtest-guide`, `sprint`, `features`, `smoke_test`, `entry_point`
- Markdown body contains step-by-step test instructions per feature
- GameForge renders these as feed cards in the project activity stream
...
| `docs/references/manifest.json` | GameForge reference manifest — structured art/audio/narrative data (when it exists) |
| `docs/sprint-logs/sprint-{N}-playtest-guide.md` | Step-by-step playtest instructions per sprint |
